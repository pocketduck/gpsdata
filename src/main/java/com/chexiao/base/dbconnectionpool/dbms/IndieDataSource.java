...
import com.chexiao.base.dbconnectionpool.jsr166.LinkedTransferQueue;
import com.chexiao.base.dbconnectionpool.jsr166.TransferQueue;
...
public class IndieDataSource extends AbstractDataSource {
    ...
    private DbDataSource createDataSource(DbConfig dbconfig) {
        ...
        if (dbds.isAlive()) {
            ...
            logger.info("创建连接成功" + dbconfig.getConnetionURL() + "耗时:" + (System.currentTimeMillis() - start));
            return dbds;
        } else {
            logger.info("创建连接失败" + dbconfig.getConnetionURL() + "耗时:" + (System.currentTimeMillis() - start));
            return null;
        }
    }
        ...
        if (currentDataSource == null || !currentDataSource.isAlive()) {
            ...
            if (dstate.isMasterDb()) {
                ...
                if (!newDataSource.getName().equals(currentDataSource.getName())) {
                    MessageAlert.Factory.get().sendMessage("写切库到:" + newDataSource.getConfig().getConnetionURL());
    ...
    private void changeMasterDb() throws SQLException {
        ...
        for (String key : managerDataSources.keySet()) {
            ...
            if (!isSameDbHost(currentDataSource.getConfig(), mcfg)) {
                ...
                try {
                    logger.debug("开始创建DBManager连接:" + mcfg.getConnetionURL());
                    mds = new DbDataSource(mcfg);
                    logger.debug("创建DBManager结果:" + mds.isAlive());
                    String dbname = getDbName(currentDataSource.getConfig());
                    DbState dstate = checkDbState(mds.getConnection(), dbname);
                    logger.info("检查数据库: " + mds.getName() + "返回状态--" + dstate.toString());
                    if (!dstate.isMasterDb() && dstate.canChange()) {
                        if (currentDataSource != null && currentDataSource.isAlive()) {
                            logger.info("发现数据库可用了，无需切换了。");
                        } else {
                            boolean state = changeMasterDb(mds.getConnection(), dbname);
                            logger.info("执行了写切库存储过程,执行状态:" + (state ? "成功" : "失败"));
                            if (state) {
                                MessageAlert.Factory.get().sendMessage("财务数据库[" + dbname + "]切库存储过程执行成功,请重新建立主从关系.");
                            }
                        }
                    }
                } finally {
    ...
    public DbDataSource getDataSource() throws SQLException {
        logger.debug("当前数据库连接：" + currentDataSource.toString());
        if (currentDataSource != null && currentDataSource.isAlive()) {
        ...
        synchronized (this) {
            logger.debug("检测前加锁 ,开始检测可用数据库连接");
            if (currentDataSource != null && currentDataSource.isAlive()) {
                return currentDataSource;
            }
            locked.set(true);
